            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <td>
                            {{-- <a href="{{ route('user.pengedarlokal.create') }}" class="btn  btn-primary">
                                <span><i class="feather icon-plus"></i> Buat Permohonan</span>
                            </a> --}}
                            <a type="button" href="{{ route('report.pengedarlokalfilter') }}" class="btn  btn-primary float-right" target="_blank">Cetak
                            </a>
                        </td>
                    </div>
                    <!-- /.card-header -->
                        ...
                        <table id="example1" class="table table-bordered table-striped">
                            ...
                            <tbody class="text-left">
                                ...
                                <tr>
                                    ...
                                    <td><a class="btn btn-info" href="{{ asset('img/pengedarbenihlokal/'.$d->persyaratan   ) }}" target="_blank">Lihat Persyaratan</a></td>
                                    <td>{{ $d->status }}</td>
                                    <td>
                                        @if ($d->status == 'Selesai')
                                        <a class="btn btn-sm btn-success text-white" target="_blank" href="{{ route('report.pengedarlokal', $d->id) }}">
                                          ...
                                          @endif
                                        <a class="btn btn-sm btn-info text-white" href="{{ route('admin.pengedarlokal.edit', $d->id) }}">
                                            <i class="fas fa-edit"></i>
                                          </a>
                                        <button data-target="#modaldelete" data-toggle="modal" type="button"
                                            class="delete btn btn-sm bg-danger"
                                            data-link="{{ route('admin.pengedarlokal.destroy',$d->id) }}">
